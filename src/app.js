...
classifier.addDocument('airships, cliffs, cable cars (aerial), Place, Tamarama Beach (N.S.W.). Transport, planes trains automobiles. Tram, Light rail, bicycle, motorbike', 'Transport');
...
var subjects = { 0:'Curios', 1:'Love', 2:'Maps', 3:'Travel', 4:'Portraits', 5:'Transport', 6:'Animals', 7:'Quotes' };

// This array converts them to vending machine columns numbers that the user will type in
var columns = { 'Curios': 1, 'Love': 2, 'Maps': 3, 'Travel': 4, 'Portraits': 5, 'Transport': 6, 'Animals': 7, 'Quotes': 8 };

// The seven rows of the vending machine are labelled thusly:
var rows = { 0: 'A', 1: 'B', 2: 'C', 3: 'D', 4: '1.', 5: '2.', 6: '3.' };
var row,column,clss;
...
stream.on('tweet', function (tweet) {
  ...
  if ((tweet.text.substring(0,2)!='RT')
   ...
   // gotta convert it to use promises like this:
  ...
  //      .catch(function (err) {
    //      console.log('caught error', err.stack)
     ...
     //     console.log('data', result.data);
      ...
      console.log('user has private tweets...'); // so pick sometninh random
      column = Math.floor(Math.random()*8);
      clss = subjects[column];
    }
    // OK do something with the results 
    row = Math.floor(Math.random()*7);
    var btn = rows[row]+columns[clss];
    var resp = 'OK @'+tweet.user.screen_name+' from the look of your tweets you might be interested in '+clss+'. Try item '+btn;
